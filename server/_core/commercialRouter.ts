...
const DEFAULT_PILLARS = [
  ...
  ["sgq_quality", "SGQ e Gestão da Qualidade", "Qualidade"],
  ["employee_experience", "Experiência do Colaborador", "Colaborador"],
...
const FEATURE_PILLAR: Record<string, string> = {
  ...
  sgq_vertical: "sgq_quality", sgq_nonconformities: "sgq_quality",
  sgq_document_process_control: "sgq_quality", sgq_audits_indicators: "sgq_quality",
  sgq_supplier_training: "sgq_quality",
  s2221_toxicological: "esocial", biometric_identity: "technology", white_label_course_library: "learning",
  employee_home: "employee_experience", employee_courses: "employee_experience",
  employee_mandatory_training: "employee_experience", employee_qualifications: "employee_experience",
  employee_first_aid: "employee_experience", employee_epi_epc: "employee_experience",
  employee_dds: "employee_experience", employee_vaccination: "employee_experience",
  employee_leaves: "employee_experience", employee_occupational_documents: "employee_experience",
  employee_certificates: "employee_experience", employee_surveys: "employee_experience",
  employee_campaigns: "employee_experience", employee_sipat: "employee_experience",
  employee_cipa: "employee_experience", employee_decompression: "employee_experience",
  employee_ethics_channel: "employee_experience", employee_manual: "employee_experience",
  employee_support: "employee_experience",
  clinic_result_updates: "integrations", provider_contracts: "governance",
  occupational_exam_reference_library: "occupational_health",
};
...
const DEFAULT_CONTRACT_CLAUSES = [
  ...
  ["prestadores_dados", "Prestadores, clinicas e responsabilidade pelos lancamentos", "prestadores", "Clinicas, laboratorios, medicos, consultores e terceiros autorizados deverao utilizar acesso proprio, manter sigilo, registrar apenas informacoes pertinentes ao atendimento contratado e responder pela veracidade dos dados, documentos, resultados, anexos e comprovantes que inserirem na plataforma.", "prestador"],
  ["prestadores_acesso", "Acesso, rastreabilidade e revogacao de terceiros", "prestadores", "O acesso de prestadores sera restrito aos clientes, funcionarios, requisicoes, resultados e documentos vinculados ao seu escopo operacional. A plataforma registrara usuario, data, hora, acao e origem dos registros, podendo o acesso ser suspenso ou revogado quando cessar a finalidade, houver descumprimento contratual ou por determinacao do contratante autorizado.", "prestador"],
...
const DEFAULT_CONTRACT_TEMPLATES = [
  ...
  ["prestador_padrao", "Contrato de Prestador / Clinica / Laboratorio", "prestador", "Modelo base para medicos, clinicas, laboratorios, consultores e terceiros autorizados", `CONTRATO DE PRESTACAO DE SERVICOS, ACESSO RESTRITO E CONFIDENCIALIDADE\n\nCONTRATADA: {{BRAND_NAME}}\nPRESTADOR/CREDENCIADO: {{CLIENT_NAME}}\nCNPJ/CPF: {{CNPJ}}\nContrato: {{CONTRACT_NUMBER}}\n\n1. ESCOPO OPERACIONAL\nPlano/escopo: {{PLAN_NAME}}\nVigencia: {{VALID_FROM}} a {{VALID_UNTIL}}\n\n2. SERVICOS E RESPONSABILIDADES\n{{FEATURES_SUMMARY}}\n\n{{CLAUSES}}\n\nENCERRAMENTO\nEste instrumento deve ser complementado pelo juridico conforme atividade exercida, conselho profissional, escopo de atendimento, responsabilidades tecnicas, confidencialidade, protecao de dados e regras de revogacao de acesso.`],
] as const;
...
export function featureMatchesAudience(feature: any, profile: string | undefined) {
  ...
  const terms: Record<string, string[]> = {
    ...
    colaborador: ["colaborador", "portal", "meus", "minhas", "dossiê", "curso", "certificado", "requisição", "aso", "epi", "dds", "vacina", "campanha", "suporte"],
    qualidade: ["qualidade", "sgq", "iso", "auditoria", "processo", "não conformidade", "fornecedor", "indicador", "competência"],
  };
  ...
  for (let index = 0; index < PLATFORM_FEATURE_MANIFEST.length; index++) {
    ...
    await db.execute(drzSql`INSERT INTO commercial_feature_catalog
      ...
      ON DUPLICATE KEY UPDATE category=VALUES(category),name=VALUES(name),description=VALUES(description),pillar_code=VALUES(pillar_code),
       ...
       integrations_json=VALUES(integrations_json),indicators_json=VALUES(indicators_json),slug=VALUES(slug),
       commercial_title=VALUES(commercial_title),headline=VALUES(headline),
       commercial_result=VALUES(commercial_result),content_version=VALUES(content_version)`);
  }
  for (const [code, feature] of Object.entries(LEGACY_COMMERCIAL_FEATURE_CONTENT)) {
    const slug = commercialSlug(code);
    const benefits = JSON.stringify(feature.benefits);
    const flow = JSON.stringify(feature.flow);
    const integrations = JSON.stringify(feature.integrations);
    const audience = JSON.stringify(feature.audience);
    await db.execute(drzSql`UPDATE commercial_feature_catalog SET
      slug=COALESCE(NULLIF(slug,''),${slug}),
      commercial_title=COALESCE(NULLIF(commercial_title,''),name),
      headline=COALESCE(NULLIF(headline,''),${feature.headline}),
      commercial_result=COALESCE(NULLIF(commercial_result,''),${feature.result}),
      benefits_json=IF(benefits_json IS NULL OR JSON_LENGTH(benefits_json)=0,${benefits},benefits_json),
      resources_json=IF(resources_json IS NULL OR JSON_LENGTH(resources_json)=0,${benefits},resources_json),
      flow_json=IF(flow_json IS NULL OR JSON_LENGTH(flow_json)=0,${flow},flow_json),
      integrations_json=IF(integrations_json IS NULL OR JSON_LENGTH(integrations_json)=0,${integrations},integrations_json),
      audience_json=IF(audience_json IS NULL OR JSON_LENGTH(audience_json)=0,${audience},audience_json),
      pillar_code=COALESCE(NULLIF(pillar_code,''),${feature.pillarCode}),
      module_name=COALESCE(NULLIF(module_name,''),category),
      content_version=IF(content_version IS NULL OR content_version IN ('','1.0'),'2.0',content_version),
      availability_status='published',available_global=1,available_white_label=1
      WHERE owner_type='global' AND owner_id=0 AND code=${code}`);
  }
...
const contractTemplateInput = z.object({
  ...
  contractType: z.enum(["saas", "white_label", "prestador", "aditivo", "distrato"]).default("saas"),
...
const contractClauseInput = z.object({
  ...
  appliesTo: z.enum(["todos", "saas", "white_label", "prestador", "aditivo", "distrato"]).default("todos"),
...
async function createCommercialPortfolioPdf(
  ...
  input: {
    ...
    pillarCodes: string[];
    featureCodes: string[];
    audienceProfile?: "diretoria" | "rh" | "sesmt" | "medico" | "engenharia" | "colaborador" | "qualidade";
  ...
  const featurePages = (await Promise.all(selectedFeatures.map(async (feature: any, index: number) => {
    ...
    const visualItems = (benefits.length ? benefits : flow).slice(0, 3);
    const screenshot = screenshotUri
      ? `<img class="screenshot" src="${screenshotUri}" alt="Tela ${esc(feature.name)}">`
      : `<div class="visual product-mock"><div class="mock-top"><i></i><i></i><i></i><b>${esc(feature.module_name || feature.category)}</b></div><div class="mock-body"><div class="mock-side"><span></span><span></span><span></span><span></span></div><div class="mock-main"><small>VISÃO OPERACIONAL</small><strong>${esc(feature.portfolio_title || feature.name)}</strong><div class="mock-kpis">${visualItems.map((item, itemIndex) => `<div><b>${String(itemIndex + 1).padStart(2, "0")}</b><span>${esc(item)}</span></div>`).join("")}</div><div class="mock-progress"><span></span></div></div></div></div>`;
  ...
  const employeeFeatures = selectedFeatures.filter((feature: any) => String(feature.pillar_code) === "employee_experience");
  const employeeJourneyPage = employeeFeatures.length ? `<section class="page-break employee-journey"><div class="kicker">Experiência do Colaborador</div><h2>O colaborador encontra tudo que é dele em um só lugar.</h2><p class="lead">Requisições, ASOs, resultados, certificados, EPI/EPC, DDS, vacinas, campanhas, pesquisas e suporte aparecem de forma individualizada, vinculada ao CPF, matrícula e perfil do trabalhador.</p><div class="employee-grid">${employeeFeatures.slice(0, 18).map((feature: any) => `<div><b>${esc(feature.portfolio_title || feature.name)}</b><span>${esc(feature.portfolio_description || feature.description)}</span></div>`).join("")}</div><div class="employee-flow">SESMT / Médico publica <b>→</b> Documento vinculado ao colaborador <b>→</b> Notificação <b>→</b> Acesso individual seguro <b>→</b> Histórico no dossiê</div></section>` : "";
  ...
  const html = `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><style>
    @page{size:A4 landscape;margin:12mm}*{box-sizing:border-box}body{margin:0;color:#183044;font-family:Arial,sans-serif;font-size:9.6pt;line-height:1.42}.cover{height:186mm;margin:-12mm;background:${primary};color:white;padding:18mm 20mm;display:flex;flex-direction:column;justify-content:space-between;page-break-after:always}.cover .brand-mark{display:inline-flex;align-items:center;background:#fff;padding:4mm 5mm;max-width:70mm;min-height:20mm;border-radius:2mm}.cover .brand-mark img{max-width:58mm;max-height:18mm;object-fit:contain;object-position:left}.cover h1{font-size:31pt;line-height:1.05;margin:8mm 0 5mm;max-width:205mm}.cover .sub{font-size:13pt;max-width:200mm;color:#e7f4f5}.cover .line{height:5px;background:${secondary};width:32mm;margin-bottom:6mm}.cover small{color:#c9d9df}.kicker{text-transform:uppercase;color:${secondary};font-weight:700;font-size:8pt;letter-spacing:1px}.page-break{page-break-before:always}h2{font-size:20pt;line-height:1.1;color:${primary};margin:3mm 0 4mm;break-after:avoid}h3{font-size:11pt;color:${primary};margin:3mm 0 2mm}.lead{font-size:10.8pt;color:#526b7b;margin-bottom:5mm}.feature-layout{display:grid;grid-template-columns:47% 53%;gap:7mm;align-items:start}.feature-copy h2{font-size:20pt}.headline{font-size:13pt;color:${secondary};line-height:1.2;margin:2mm 0 4mm}.feature-copy h4{margin:5mm 0 2mm;text-transform:uppercase;font-size:7.5pt;color:#6b7f8d}.benefits{padding-left:5mm;margin:0}.benefits li{margin:1.6mm 0}.visual{height:83mm;background:${primary};color:white;overflow:hidden}.product-mock{display:block;padding:0;background:#eef4f6;border:1px solid #cbd9df;color:${primary}}.mock-top{height:10mm;background:${primary};display:flex;align-items:center;gap:2mm;padding:0 4mm;color:white}.mock-top i{width:2.2mm;height:2.2mm;border-radius:50%;background:${secondary}}.mock-top b{margin-left:2mm;font-size:8pt}.mock-body{height:73mm;display:grid;grid-template-columns:12mm 1fr}.mock-side{background:#dfe9ed;padding-top:6mm}.mock-side span{display:block;width:5mm;height:5mm;margin:0 auto 4mm;border-radius:1mm;background:#9fb3bd}.mock-main{padding:6mm}.mock-main small{display:block;color:#718794;font-size:7pt;margin:0 0 2mm}.mock-main>strong{display:block;color:${primary};font-size:13pt;line-height:1.15;min-height:13mm}.mock-kpis{display:grid;grid-template-columns:1fr;gap:2mm;margin-top:4mm}.mock-kpis div{display:grid;grid-template-columns:9mm 1fr;align-items:center;min-height:11mm;background:white;border-left:3px solid ${secondary};padding:2mm}.mock-kpis b{font-size:10pt;color:${secondary}}.mock-kpis span{font-size:7.3pt;color:#385367;font-weight:600}.mock-progress{height:3mm;background:#d3e0e5;margin-top:5mm}.mock-progress span{display:block;width:72%;height:100%;background:${secondary}}.screenshot{width:100%;height:83mm;object-fit:cover;object-position:top;border:1px solid #d4e0e5}.flow,.ecosystem{display:flex;flex-wrap:wrap;gap:2mm;margin-top:5mm;background:#edf6f7;padding:4mm;color:${primary};font-weight:700}.flow b,.integration-row b{width:100%;font-size:7.5pt;text-transform:uppercase;color:#6b7f8d}.integration-row{display:flex;flex-wrap:wrap;gap:2mm;margin-top:4mm}.integration-row span,.pill-list span{border:1px solid #c9d9df;padding:2mm 3mm;font-size:7.8pt}.result{border-left:4px solid ${secondary};margin-top:4mm;padding:2.5mm 4mm;background:#f5f8f9}.pillar-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:2.5mm}.pillar-grid div{border-left:4px solid ${secondary};background:#f2f6f8;padding:3mm}.pillar-grid span,.pillar-grid small{display:block;color:#6b7f8d;font-size:7.8pt}.pillar-grid b{display:block;color:${primary};font-size:10.5pt;margin:1mm 0}.before-after{display:grid;grid-template-columns:.75fr 1.25fr;gap:6mm;margin-top:9mm}.before-after>div{border-top:5px solid #c6d1d6;padding:5mm;background:#f5f7f8}.before-after .connected{border-color:${secondary};background:#edf7f7}.connected div{margin-top:5mm;font-size:11pt;font-weight:700;line-height:1.6}.impact-page{min-height:186mm;background:${primary};color:white;margin:-12mm;padding:22mm 20mm;display:flex;flex-direction:column;justify-content:center}.impact-page h2{color:white;font-size:29pt;max-width:210mm}.impact-page p{font-size:13pt;color:#d9e7ea;max-width:205mm}.impact-flow,.employee-flow{margin-top:8mm;border-top:4px solid ${secondary};padding-top:6mm;font-size:13pt;font-weight:700}.pill-list{display:flex;flex-wrap:wrap;gap:3mm;margin-top:8mm}.pill-list span{border-color:#6d8793}.employee-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:2.5mm;margin-top:5mm}.employee-grid div{background:#f2f7f8;border-left:4px solid ${secondary};padding:3mm;min-height:20mm}.employee-grid b{display:block;color:${primary};font-size:9.5pt}.employee-grid span{display:block;color:#526b7b;font-size:7.8pt;margin-top:1mm}.callout{margin-top:8mm;background:${primary};color:white;padding:7mm;font-size:12pt}.footer{margin-top:8mm;border-top:1px solid #d7e2e7;padding-top:4mm;color:#6b7f8d;font-size:8pt}
  </style></head><body><div class="cover"><div>${logo ? `<span class="brand-mark"><img src="${logo}"></span>` : `<strong style="font-size:22pt">${esc(brand.brand_name || "Plataforma")}</strong>`}</div><div><div class="line"></div><div class="kicker" style="color:${secondary}">${esc(input.title)}</div><h1>Sua operação de SST, conectada de ponta a ponta.</h1><div class="sub">${esc(input.presentationText || "Saúde, Segurança, Pessoas, Conformidade e Inteligência em uma única plataforma. Tudo conectado. Tudo rastreável.")}</div></div><small>Base comercial ${esc(PLATFORM_RELEASE.code)} · ${selectedFeatures.length} funcionalidades · ${new Date().toLocaleDateString("pt-BR")}</small></div>${whyPlatformPage}${differentiatorPage}${whiteLabelPage}${pillarPage}${employeeJourneyPage}${featurePages}<section class="page-break"><div class="kicker">Próximo passo</div><h2>Vamos transformar a gestão de SST?</h2><p class="lead">Apresente o cenário da sua organização e conheça uma jornada de implantação adequada às suas necessidades.</p><div class="callout"><b>Solicitar demonstração</b><br><br>${esc(input.consultantName || brand.contact_name || brand.brand_name || "Contato comercial")}<br>${esc(input.consultantContact || brand.contact_email || "")}${brand.contact_phone ? ` · ${esc(brand.contact_phone)}` : ""}<br>${esc(brand.website || "")}</div><div class="footer">Portfólio gerado pela Base Global de Funcionalidades. O escopo contratual é definido na proposta comercial.</div></section></body></html>`;
  const puppeteer = (await import("puppeteer")).default;
...
export const commercialRouter = router({
  ...
  generatePortfolio: commercialProcedure.input(z.object({
    ...
    audienceProfile: z.enum(["diretoria","rh","sesmt","medico","engenharia","colaborador","qualidade"]).optional(),
