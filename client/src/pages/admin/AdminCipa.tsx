...
function ElectionsTab() {
  ...
  return (
    <div className="space-y-4 mt-4">
      ...
      {selected && <ElectionDossierPanel electionId={selected.id} />}
...
function ElectionDossierPanel({ electionId }: { electionId: number }) {
  const previewQ = (trpc.cipa as any).electionDossierPreview.useQuery({ electionId });
  const generateMut = (trpc.cipa as any).generateElectionDossier.useMutation({
    onSuccess: async (result: any) => { if (result.url) window.open(result.url, "_blank"); toast.success(`Dossiê eleitoral ${result.protocolCode} gerado.`); await previewQ.refetch(); },
    onError: (error: any) => toast.error(error?.message ?? "Não foi possível gerar o dossiê eleitoral."),
  });
  const data = previewQ.data as any;
  if (!data) return null;
  return <section className="border border-slate-200 bg-white p-5">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div><h3 className="flex items-center gap-2 font-bold text-slate-900"><FileText size={17} className="text-teal-700" /> Dossiê do processo eleitoral</h3><p className="mt-1 font-mono text-xs font-bold text-teal-800">Protocolo interno: {data.protocolCode || "em processamento"}</p><p className="mt-1 max-w-2xl text-xs text-slate-500">Consolida as evidências do processo sem relacionar o eleitor ao candidato escolhido.</p></div>
      <button onClick={() => generateMut.mutate({ electionId })} disabled={generateMut.isPending} className="inline-flex items-center gap-2 rounded-md bg-teal-700 px-3 py-2 text-sm font-semibold text-white hover:bg-teal-800 disabled:opacity-50">{generateMut.isPending ? <Loader2 size={15} className="animate-spin" /> : <FileText size={15} />} Gerar dossiê em PDF</button>
    </div>
    <div className="mt-4 grid grid-cols-2 gap-2 md:grid-cols-5">{[["Elegíveis", data.totalEligible], ["Votantes", data.totalVoters], ["Votos apurados", data.totalVotes], ["Candidatos", data.totalCandidates], ["Participação", `${data.turnout}%`]].map(([label, value]) => <div key={String(label)} className="border-l-4 border-teal-600 bg-slate-50 p-3"><span className="block text-xs text-slate-500">{label}</span><b className="text-xl text-slate-900">{value}</b></div>)}</div>
    <p className="mt-3 border-l-4 border-amber-400 bg-amber-50 p-2 text-xs text-amber-900">O protocolo facilita consulta e auditoria interna. Ele não representa homologação governamental; a conformidade depende do cumprimento material da NR-05 e das regras coletivas aplicáveis.</p>
  </section>;
}
