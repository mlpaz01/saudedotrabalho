...
const DOCS_OFICIAIS = [
  {
    titulo: "NR-01 — Normas Regulamentadoras Vigentes",
    descricao: "Texto oficial vigente da NR-01 (Disposições Gerais e GRO)",
  ...
  },
  {
    ...
    descricao:
      "Altera a NR-01 incluindo os fatores de risco psicossociais (PDF oficial)",
  ...
  {
    titulo: "Guia NR-01 Revisado — Riscos Psicossociais",
    descricao:
      "Guia prático do MTE sobre identificação e gestão dos riscos psicossociais (PDF oficial)",
  ...
  {
    titulo: "Perguntas e Respostas — NR-01",
    descricao: "FAQ oficial do Ministério do Trabalho e Emprego sobre a NR-01",
  ...
  {
    titulo: "Manual GRO / PGR",
    descricao: "Portal oficial de Segurança e Saúde no Trabalho (GRO/PGR)",
    url: "https://www.gov.br/trabalho-e-emprego/pt-br/assuntos/inspecao-do-trabalho/seguranca-e-saude-no-trabalho",
    tag: "GRO/PGR",
  },
  {
    titulo: "Inclusão dos Riscos Psicossociais na NR-01",
    descricao: "Comunicado oficial do MTE sobre a atualização da norma",
    url: "https://www.gov.br/trabalho-e-emprego/pt-br/noticias-e-conteudo/2024/Setembro/governo-federal-atualiza-nr-01-para-incluir-riscos-psicossociais-e-reconstitui-comissao-do-benzeno",
    tag: "Atualização",
  },
];
...
function ComplianceSeal({ score }: { score: number }) {
  ...
  return (
    <div className="flex flex-wrap gap-2 mt-3">
      ...
      <p className="w-full text-xs text-slate-400 mt-0.5">
        Selos calculados com base em evidências reais na plataforma. Não
        representam certificação oficial.
      </p>
    </div>
  );
}
      ...
      <div className="bg-white rounded-xl border p-5 flex items-start justify-between gap-4">
        <div>
          <h2 className="font-semibold text-lg mb-1">
            Relatório de Primeiros Socorros
          </h2>
          <p className="text-sm text-slate-500">
            Evidência de kits, validade, itens faltantes, estoque e distribuição
            por filial/setor.
          </p>
        </div>
        <Button
              ...
              <table className="w-full text-xs">
                <thead className="bg-slate-50 text-slate-500">
                  <tr>
                    ...
                    <th className="p-2">30 dias</th>
                    <th className="p-2">Faltantes</th>
                    <th className="p-2">Críticos</th>
                  </tr>
                </thead>
                <tbody>
                  {(d?.byLocation ?? []).map((r: any, i: number) => (
                    <tr
                      key={`${r.branchName}-${r.sectorName}-${i}`}
                      className="border-t"
                    >
                      <td className="p-2">{r.branchName}</td>
                      <td className="p-2">{r.sectorName}</td>
                      <td className="p-2 text-center">{r.kits}</td>
                      <td className="p-2 text-center">{r.items}</td>
                      <td className="p-2 text-center">{r.expired}</td>
                      <td className="p-2 text-center">{r.soon30}</td>
                      <td className="p-2 text-center">{r.missing}</td>
                      <td className="p-2 text-center">{r.critical}</td>
                    </tr>
                  ))}
                  {!(d?.byLocation ?? []).length && (
                    <tr>
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white rounded-xl border p-5 flex items-start justify-between gap-4">
        <div>
          <h2 className="font-semibold text-lg mb-1">
            Gestão de EPI / EPC — NR-06
          </h2>
          <p className="text-sm text-slate-500">
            Evidência de CA, validade do produto, vínculo PGR/GSE, entregas,
            ...
            <table className="w-full text-xs">
              <thead className="bg-slate-50 text-slate-500">
                <tr>
                  <th className="text-left p-2">Filial</th>
                  <th className="text-left p-2">Setor</th>
                  <th className="p-2">EPIs</th>
                  <th className="p-2">EPCs</th>
                  <th className="p-2">CA vencido</th>
                  <th className="p-2">Produto vencido</th>
                  <th className="p-2">Estoque crítico</th>
                </tr>
              </thead>
              <tbody>
                {(d?.byLocation ?? []).map((r: any, i: number) => (
                  <tr key={i} className="border-t">
                    <td className="p-2">{r.branchName}</td>
                    <td className="p-2">{r.sectorName}</td>
                    <td className="p-2 text-center">{r.epis}</td>
                    <td className="p-2 text-center">{r.epcs}</td>
                    <td className="p-2 text-center">{r.caExpired}</td>
                    <td className="p-2 text-center">{r.productExpired}</td>
                    <td className="p-2 text-center">{r.stockCritical}</td>
                  </tr>
                ))}
                {!(d?.byLocation ?? []).length && (
                  <tr>
...
function OccupationalLifecycleCompliancePanel() {
  ...
  const conformityQ =
    trpc.occupationalLifecycle.occupationalConformity.useQuery(filters);
  const data = dashboardQ.data as any;
  const conformity = conformityQ.data as any;
  const summary = conformity?.summary || {};
  const cards = [
    [
      "Cobertura GSE",
      `${Number(data?.gse_coverage || 0)}%`,
      Number(data?.workers_without_gse || 0) === 0,
    ],
    ["Exames previstos", Number(summary.complementaryExpected || 0), true],
    [
      "Exames pendentes",
      Number(summary.complementaryPending || 0),
      Number(summary.complementaryPending || 0) === 0,
    ],
    [
      "Conclusão dos exames",
      `${Number(summary.complementaryRate || 0)}%`,
      Number(summary.complementaryRate || 0) === 100,
    ],
    ["ASOs previstos", Number(summary.clinicalExpected || 0), true],
    [
      "ASOs pendentes",
      Number(summary.clinicalPending || 0),
      Number(summary.clinicalPending || 0) === 0,
    ],
  ];
  const setFilter = (key: string, value: any) =>
    setFilters((current: any) => ({ ...current, [key]: value || undefined }));
  const exportDefaulters = () => {
    const rows = [
      ...(conformity?.complementaryDefaulters || []).map((row: any) => ({
        ...row,
        category: "Exame complementar",
      })),
      ...(conformity?.clinicalAsoDefaulters || []).map((row: any) => ({
        ...row,
        category: "Avaliação clínica / ASO",
      })),
    ];
    const keys = [
      "category",
    ...
    anchor.href = url;
    anchor.download = `faltosos_saude_ocupacional_${filters.year}.csv`;
    anchor.click();
    URL.revokeObjectURL(url);
  };
  const Defaulters = ({ title, rows }: { title: string; rows: any[] }) => (
    <section className="border bg-white">
      <div className="border-b bg-slate-50 px-4 py-3">
        <h3 className="font-semibold">{title}</h3>
        <p className="text-xs text-slate-500">
          {rows.length} pendência(s) neste recorte.
        </p>
      </div>
        ...
        <table className="w-full min-w-[980px] text-xs">
          <thead className="sticky top-0 bg-white text-slate-500">
            <tr>
              ...
              <th className="p-2 text-left">Cargo / GSE</th>
              <th className="p-2 text-left">Serviço</th>
              <th className="p-2 text-left">Pendência</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row, index) => (
              <tr
                className="border-t"
                key={`${row.collaborator_id}-${row.exam_id || 0}-${index}`}
              >
                <td className="p-2">
                ...
                <td className="p-2">
                  {row.branch_name || "Sem filial"}
                  <br />
                  {row.sector_name || "Sem setor"}
                </td>
                <td className="p-2">
                ...
                <td className="p-2 text-amber-800">{row.pendingReason}</td>
              </tr>
            ))}
            {!rows.length && (
              <tr>
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3 border bg-white p-5">
        <div>
          <h2 className="text-lg font-semibold">
            Conformidade da Saúde Ocupacional
          </h2>
          <p className="mt-1 text-sm text-slate-500">
            Cadeia rastreável: GSE, PGR, PCMSO, obrigação, realização, anamnese
            e ASO finalizado.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <a
            className="inline-flex h-10 items-center border px-4 text-sm font-semibold text-teal-800"
      ...
      <div className="grid gap-3 border bg-white p-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7">
        ...
        <label className="text-xs font-semibold">
          Filial
          <select
            className="mt-1 h-10 w-full border bg-white px-2"
            value={filters.branchId || ""}
            onChange={event =>
              setFilter("branchId", Number(event.target.value))
            }
          >
            <option value="">Todas</option>
            {(conformity?.filters?.branches || []).map((row: any) => (
        ...
        <label className="text-xs font-semibold">
          ...
          <select
            ...
            onChange={event =>
              setFilter("sectorId", Number(event.target.value))
            }
          >
            <option value="">Todos</option>
            {(conformity?.filters?.sectors || []).map((row: any) => (
              <option value={row.id} key={row.id}>
                {row.name}
              </option>
            ))}
        ...
        <label className="text-xs font-semibold">
          ...
          <select
            className="mt-1 h-10 w-full border bg-white px-2"
            value={filters.gseId || ""}
            onChange={event => setFilter("gseId", Number(event.target.value))}
          >
            <option value="">Todos</option>
            {(conformity?.filters?.gses || []).map((row: any) => (
              <option value={row.id} key={row.id}>
                {row.name}
              </option>
            ))}
        ...
        <label className="text-xs font-semibold">
          Cargo
          <input
            className="mt-1 h-10 w-full border px-2"
        ...
        <label className="text-xs font-semibold">
          ...
          <select
            ...
            onChange={event => setFilter("examId", Number(event.target.value))}
          >
            <option value="">Todos</option>
            {(conformity?.filters?.exams || []).map((row: any) => (
              <option value={row.id} key={row.id}>
                {row.name}
              </option>
            ))}
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
        {cards.map(([label, value, compliant]) => (
          <div
            key={String(label)}
            className={`border p-4 ${compliant ? "border-emerald-200 bg-emerald-50" : "border-amber-200 bg-amber-50"}`}
          >
            ...
            <div
              className={`mt-2 text-xs font-semibold ${compliant ? "text-emerald-700" : "text-amber-700"}`}
            >
              {compliant ? "Conforme" : "Requer atenção"}
            </div>
          </div>
        ))}
      </div>
      <div className="grid gap-3 md:grid-cols-2">
        <div className="border-l-4 border-teal-500 bg-teal-50 p-4">
          <b>Exames complementares: {summary.complementaryRate || 0}%</b>
          <p className="mt-1 text-xs">
            {summary.complementaryCompleted || 0} concluídos de{" "}
            {summary.complementaryExpected || 0} previstos.
          </p>
        </div>
        <div className="border-l-4 border-sky-500 bg-sky-50 p-4">
          <b>Avaliação clínica e ASO: {summary.clinicalRate || 0}%</b>
          <p className="mt-1 text-xs">
            {summary.clinicalCompleted || 0} finalizados de{" "}
            {summary.clinicalExpected || 0} previstos.
          </p>
        </div>
      </div>
      <Defaulters
        title="Faltosos de exames complementares"
        rows={conformity?.complementaryDefaulters || []}
      />
      <Defaulters
        title="Faltosos de avaliação clínica, anamnese e ASO"
        rows={conformity?.clinicalAsoDefaulters || []}
      />
      <div className="border-l-4 border-sky-500 bg-sky-50 p-4 text-sm">
        {conformity?.note} Eventos eSocial permanecem pendentes até que a
        integração oficial seja habilitada.
      </div>
    </div>
  );
}
            ...
            <section className="border bg-white p-4" key={label}>
              ...
              {checks.length ? (
                ...
                <p className="mt-3 border border-dashed p-3 text-xs text-slate-500">
                  Execute a auditoria no documento para gerar o checklist por
                  requisito.
                </p>
              )}
            </section>
...
export default function ComplianceHub() {
  ...
  const dossieQ = trpc.audit.evidenceReport.useQuery(
    { userId: dossieUserId ?? 0 },
    { enabled: !!dossieUserId }
  );
      ...
      <div className="p-6 max-w-6xl mx-auto space-y-5">
        ...
        <div className="relative pl-4">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              >
                Central de Conformidade NR-01
              </h1>
              <p className="text-sm text-slate-500">
                Prontidão para fiscalizações · Evidências auditáveis · Gestão
                dos 13 fatores de risco psicossocial
              </p>
            </div>
          </div>
        </div>
              ...
              <>
                <div className="bg-white rounded-xl border p-6 flex flex-col md:flex-row items-center gap-8">
                  ...
                  <div className="flex-1 min-w-0">
                    <h2 className="font-semibold text-lg mb-1">
                      Índice de Conformidade NR-01
                    </h2>
                    <p className="text-sm text-slate-500 mb-3">
                      Calculado automaticamente: ciclos ativos, inventário,
                      plano de ação, participação nas pesquisas, treinamentos e
                    ...
                    return (
                      ...
                      >
                        <button
                          ...
                          onClick={() =>
                            setExpandedAxis(isOpen ? null : ax.axis)
                          }
                        >
                          <div
                            className={`w-9 h-9 rounded-lg flex items-center justify-center bg-${c}-50 flex-shrink-0`}
                          >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div
                              className={`w-full bg-slate-100 rounded-full h-1.5 mt-1.5`}
                            >
                              <div
                                className={`h-1.5 rounded-full bg-${c}-500 transition-all`}
                                style={{ width: `${ax.score}%` }}
                              />
                            </div>
                          </div>
                          <span
                            className={`text-sm font-bold text-${c}-600 w-10 text-right flex-shrink-0`}
                          >
                            {ax.score}%
                          </span>
                          {isOpen ? (
                ...
                {(st?.ranking ?? []).length > 0 && (
                  <div className="bg-white rounded-xl border p-5">
                    <h3 className="font-semibold text-sm mb-3 flex items-center gap-2">
                      <Building2 size={15} /> Ranking de Conclusão por Setor
                    </h3>
                    <div className="space-y-2">
                      {(st!.ranking as any[]).map((r: any, i: number) => (
                        ...
                        >
                          <span className="text-slate-400 w-5 text-right text-xs">
                            {i + 1}.
                          </span>
            ...
            {maturityQ.isLoading ? (
              ...
              <>
                <div className="bg-white rounded-xl border p-6 flex flex-col md:flex-row items-center gap-8">
                  ...
                  <div className="flex-1 min-w-0">
                    <h2 className="font-semibold text-lg mb-1">
                      Índice de Maturidade em Conformidade
                    </h2>
                    <p className="text-sm text-slate-500">
                      {maturityQ.data.company?.name} — visão executiva
                      combinando pesquisas, treinamentos prioritários,
                      evidências documentais, plano de ação, CIPA, SIPAT e
                      primeiros socorros. Pensado para reuniões com clientes e
                      auditorias.
                    </p>
                  </div>
                </div>
                      ...
                      return (
                        <div key={ax.key} className="flex items-center gap-3">
                          ...
                          <div className="flex-1 bg-slate-100 rounded-full h-2.5">
                            <div
                              className={`h-2.5 rounded-full bg-${c}-500`}
                              style={{ width: `${ax.score}%` }}
                            />
                          </div>
                          <span
                            className={`text-sm font-bold w-12 text-right text-${c}-600`}
                          >
                            {ax.score}%
                          </span>
                        </div>
                ...
                <div className="bg-white rounded-xl border p-5 text-xs text-slate-500 grid grid-cols-2 md:grid-cols-4 gap-3">
                  <div>
                    <span className="text-slate-400">DRPS respondido:</span>{" "}
                    {maturityQ.data.details.drpsRespondido ? "Sim" : "Não"}
                  </div>
                  <div>
                    <span className="text-slate-400">AEP respondido:</span>{" "}
                    {maturityQ.data.details.aepRespondido ? "Sim" : "Não"}
                  </div>
                  <div>
                    <span className="text-slate-400">
                      Participação pesquisas:
                    </span>{" "}
                    {maturityQ.data.details.respondentes}/
                    {maturityQ.data.details.totalEmp}
                  </div>
                  <div>
                    <span className="text-slate-400">Ações no prazo:</span>{" "}
          ...
          <div className="space-y-5">
            <div className="bg-white rounded-xl border p-6">
              <h2 className="font-semibold text-lg mb-2">
                Simulação de Fiscalização NR-01
              </h2>
              <p className="text-sm text-slate-500 mb-4">
                Analisa automaticamente os mesmos 6 eixos verificados por um
                Auditor Fiscal do Trabalho em fiscalizações de riscos
                psicossociais: inventário, plano de ação, participação, fatores
                de risco, capacitação e evidências.
              </p>
              <div className="flex flex-wrap gap-3">
                ...
                <Button
                  ...
                  onClick={() =>
                    window.open(
                      "/plataforma/admin/compliance/relatorio-fiscalizacao",
                      "_blank"
                    )
                  }
                >
                ...
                <Button
                  ...
                  onClick={() =>
                    window.open(
                      "/plataforma/admin/compliance/relatorio-metodologia",
                      "_blank"
                    )
                  }
                >
                  <FileText size={15} /> Gerar Relatório de Legitimidade
                  Metodológica
                </Button>
              </div>
              ...
              <div className="mt-3 pt-3 border-t flex flex-wrap gap-2">
                <Button
                  ...
                  onClick={() =>
                    window.open(
                      "/plataforma/admin/compliance/canal-denuncias",
                      "_blank"
                    )
                  }
                >
                ...
                <Button
                  ...
                  onClick={() =>
                    window.open(
                      "/plataforma/admin/compliance/lei-14457",
                      "_blank"
                    )
                  }
                >
                ...
                <Button
                  ...
                  className="gap-2"
                  onClick={() =>
                    window.open("/plataforma/admin/compliance/lgpd", "_blank")
                  }
                >
            ...
            {simRunning && (
              <div className="text-center py-12 bg-white rounded-xl border">
                ...
                <p className="font-medium text-slate-700">
                  Cruzando dados da empresa com os critérios da NR-01...
                </p>
                <p className="text-xs text-slate-400 mt-1">
                  Inventário · Plano de ação · Participação · Capacitação ·
                  Evidências
                </p>
              </div>
            )}
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-3 gap-3">
                  {[
                    ...
                    <div
                      key={key}
                      className={`bg-${bg}-50 border border-${bg}-200 rounded-xl p-4 text-center`}
                    >
                      <Icon
                        size={22}
                        className={`mx-auto text-${bg}-600 mb-1`}
                      />
                      <div className={`text-2xl font-bold text-${bg}-700`}>
                        {(simResult[key] ?? []).length}
                      </div>
                      <div className={`text-xs font-medium text-${bg}-600`}>
                        {label}
                      </div>
                    </div>
                  ))}
                ...
                {[
                  ...
                  return (
                    <div
                      key={key}
                      className={`bg-white rounded-xl border border-${c}-200 overflow-hidden`}
                    >
                      <div
                        className={`px-5 py-3 border-b border-${c}-100 bg-${c}-50`}
                      >
                        <h3 className={`font-semibold text-${c}-800 text-sm`}>
                          {title} ({items.length})
                        </h3>
                      </div>
                      <div className="divide-y">
                        {items.map((item: any, i: number) => (
                          <div key={i} className="px-5 py-3">
                            ...
                            {item.acao && (
                              <div
                                className={`text-xs text-${c}-700 mt-1 font-medium`}
                              >
                                💡 {item.acao}
                              </div>
                            )}
        ...
        {tab === "evidencias" && (
          <div className="space-y-4">
            <div className="bg-white rounded-xl border p-5">
              <h2 className="font-semibold text-lg mb-1">
                Evidências Vinculadas NR-01
              </h2>
              <p className="text-sm text-slate-500">
                Registros auditáveis gerados automaticamente — disponíveis para
                fiscalizações e processos judiciais.
              </p>
            </div>
            {evidQ.isLoading ? (
              <div className="flex justify-center py-8">
        ...
        {tab === "occupational_lifecycle" && (
          <OccupationalLifecycleCompliancePanel />
        )}
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-xl border p-5 print:hidden">
              <h2 className="font-semibold text-lg mb-3">
                Dossiê Individual do Colaborador
              </h2>
              <p className="text-sm text-slate-500 mb-4">
                Trilha completa por colaborador: cursos, certificados, aceites
                eletrônicos (com IP e timestamp), tentativas de quiz e trilha de
                auditoria.
              </p>
                  ...
                  <div className="space-y-4">
                    <div className="bg-white rounded-xl border p-5">
                      <h3 className="font-semibold mb-3 text-sm flex items-center gap-2">
                        <Users size={14} /> Dados do Colaborador
                      </h3>
                    ...
                    <div className="bg-white rounded-xl border p-5">
                      ...
                      <h3 className="font-semibold mb-3 text-sm flex items-center gap-2">
                        <GraduationCap size={14} /> Cursos Concluídos (
                        {r.completedModules?.length ?? 0})
                      </h3>
                      {!r.completedModules?.length ? (
                        ...
                        <table className="w-full text-xs">
                          <thead className="bg-slate-50 text-left">
                            <tr>
                              ...
                              <th className="p-2">Concluído em</th>
                              <th className="p-2">Certificado</th>
                            </tr>
                          </thead>
                          <tbody>
                            {r.completedModules.map((m: any) => {
                              const cert = (r.certs ?? []).find(
                                (c: any) =>
                                  Number(c.moduleId) === Number(m.moduleId)
                              );
                              return (
                    ...
                    <div className="bg-white rounded-xl border p-5">
                      <h3 className="font-semibold mb-3 text-sm flex items-center gap-2">
                        <CheckCircle2 size={14} /> Aceites Eletrônicos (
                        {r.acceptances?.length ?? 0})
                      </h3>
                      {!r.acceptances?.length ? (
                        <p className="text-sm text-slate-400">
                    ...
                    <div className="bg-white rounded-xl border p-5">
                      <h3 className="font-semibold mb-3 text-sm flex items-center gap-2">
                        <ClipboardCheck size={14} /> Avaliações (
                        {r.attempts?.length ?? 0})
                      </h3>
                      {!r.attempts?.length ? (
                        <p className="text-sm text-slate-400">
                    ...
                    <div className="bg-white rounded-xl border p-5">
                      <h3 className="font-semibold mb-3 text-sm flex items-center gap-2">
                        <FileSearch size={14} /> Trilha de Auditoria
                      </h3>
                      <table className="w-full text-xs">
                        <thead className="bg-slate-50 text-left">
                          <tr>
        ...
        {tab === "documentos" && (
          <div className="space-y-4">
            <div className="bg-white rounded-xl border p-5">
              <h2 className="font-semibold text-lg mb-1">
                Documentos Oficiais NR-01
              </h2>
              <p className="text-sm text-slate-500">
                Links diretos para fontes oficiais do Ministério do Trabalho e
                Emprego relacionados à NR-01.
              </p>
            </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <h3 className="font-semibold text-indigo-900 mb-1">
                    Metodologia de Coleta de Dados Psicossociais
                  </h3>
                  <p className="text-xs text-indigo-800 leading-relaxed mb-2">
                    Duas vias operacionais oficiais, em conformidade com NR-01,
                    Lei 14.457/2022 e LGPD:
                  </p>
                  <ul className="text-xs text-indigo-900 space-y-1 list-disc pl-4 mb-2">
                    <li>
                      <b>Digital:</b> DRPS/MBI online na plataforma, respostas
                      anônimas agregadas por setor.
                    </li>
                    <li>
                      <b>Impressa:</b> impressão → aplicação → lançamento em
                      lote pelo RH com <b>Termo de Confidencialidade</b>{" "}
                      eletrônico (nome+CPF+cargo),{" "}
                      <b>Recibo de Confidencialidade</b> em PDF com código único
                      + QR Code de rastreabilidade fixado externamente ao
                      envelope lacrado, e integração automática no banco como
                      respostas anônimas.
                    </li>
                  </ul>
                  <p className="text-xs text-indigo-700">
                    Esta metodologia é <b>renderizada automaticamente</b> em
                    todo PGR emitido pela plataforma, garantindo documentação
                    técnica consistente para auditoria externa.
                  </p>
                </div>
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {DOCS_OFICIAIS.map(doc => (
                <div
                  key={doc.titulo}
                  className="bg-white rounded-xl border p-5 flex gap-4 hover:border-emerald-300 transition-colors"
                >
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-xs text-amber-800">
              <strong>Aviso:</strong> Links apontam para documentos do
              Ministério do Trabalho e Emprego. Se algum link estiver
              indisponível, acesse diretamente{" "}
              <a
