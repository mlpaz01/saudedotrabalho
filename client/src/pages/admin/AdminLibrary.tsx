...
import {
  Library, BookOpen, ClipboardList, Leaf, Sparkles, Copy, Wand2,
  CheckCircle2, Brain, ShieldAlert, BarChart3, Clock, Loader2, Search,
  Wind, Eye, Zap, Dumbbell,
} from "lucide-react";
...
.ucg-card { background: #fff; border: 1.5px solid #E9EDF2; border-radius: 16px; overflow: hidden; cursor: pointer; transition: box-shadow .18s, transform .18s, border-color .18s; display: flex; flex-direction: column; }
...
.ucg-thumb-icon { display: flex; align-items: center; justify-content: center; width: 54px; height: 54px; border-radius: 16px; background: rgba(255,255,255,.25); backdrop-filter: blur(4px); }
.ucg-thumb-icon svg { width: 28px; height: 28px; stroke: #fff; fill: none; stroke-width: 1.8; }
...
.ucg-footer-btn { flex: 1; padding: 7px 10px; border-radius: 9px; font-size: 12px; font-weight: 600; border: none; cursor: pointer; transition: all .14s; display: flex; align-items: center; justify-content: center; gap: 5px; }
...
.ucg-footer-ghost { background: #F4F6F9; color: #0E2C46; }
...
const SURVEY_CFG: Record<string, { label: string; gradient: string; catStyle: string; Icon: any }> = {
  psicossocial: { label: "DRPS — quantitativa", gradient: "linear-gradient(135deg,#3B82F6,#1D4ED8)", catBg: "rgba(59,130,246,.12)", catColor: "#1D4ED8", Icon: Brain },
  aep:          { label: "AEP — qualitativa",   gradient: "linear-gradient(135deg,#A855F7,#7C3AED)", catBg: "rgba(168,85,247,.12)", catColor: "#7C3AED", Icon: ClipboardList },
  clima:        { label: "Clima",               gradient: "linear-gradient(135deg,#10B981,#059669)", catBg: "rgba(16,185,129,.12)", catColor: "#059669", Icon: BarChart3 },
  burnout:      { label: "Burnout",             gradient: "linear-gradient(135deg,#F59E0B,#D97706)", catBg: "rgba(245,158,11,.12)", catColor: "#D97706", Icon: ShieldAlert },
  assedio:      { label: "Assédio",             gradient: "linear-gradient(135deg,#EF4444,#DC2626)", catBg: "rgba(239,68,68,.12)", catColor: "#DC2626", Icon: ShieldAlert },
};

const DECOMP_CFG: Record<string, { label: string; gradient: string; catStyle: string; Icon: any }> = {
  breathing:  { label: "Respiração",  gradient: "linear-gradient(135deg,#38BDF8,#0284C7)", catBg: "rgba(56,189,248,.12)", catColor: "#0284C7",  Icon: Wind },
  meditation: { label: "Meditação",   gradient: "linear-gradient(135deg,#A78BFA,#7C3AED)", catBg: "rgba(167,139,250,.12)", catColor: "#7C3AED", Icon: Eye },
  stretching: { label: "Alongamento", gradient: "linear-gradient(135deg,#34D399,#059669)", catBg: "rgba(52,211,153,.12)", catColor: "#059669",  Icon: Dumbbell },
  reflection: { label: "Reflexão",    gradient: "linear-gradient(135deg,#FCD34D,#D97706)", catBg: "rgba(252,211,77,.12)", catColor: "#D97706",  Icon: Eye },
  focus:      { label: "Foco",        gradient: "linear-gradient(135deg,#818CF8,#4338CA)", catBg: "rgba(129,140,248,.12)", catColor: "#4338CA", Icon: Brain },
  energy:     { label: "Energia",     gradient: "linear-gradient(135deg,#FB923C,#EA580C)", catBg: "rgba(251,146,60,.12)", catColor: "#EA580C",  Icon: Zap },
};

const MODULE_GRADIENT = "linear-gradient(135deg,#F59E0B,#D97706)";
const MODULE_CAT_STYLE = "background:rgba(245,158,11,.12);color:#D97706";
  ...
  return (
    <>
      {Array.from({ length: 8 }).map((_, i) => (
        <div key={i} className="ucg-card" style={{ minHeight: 260 }}>
          <div className="ucg-skel" style={{ height: 140 }} />
            ...
            {!listQ.isLoading && tab === "surveys" && (
              visibleSurveys.length === 0
                ...
                : visibleSurveys.map((s: any) => {
                  const cfg = SURVEY_CFG[s.category] ?? { label: s.category ?? "Custom", gradient: "linear-gradient(135deg,#64748b,#475569)", catBg: "rgba(100,116,139,.12)", catColor: "#475569", Icon: ClipboardList };
                  const Icon = cfg.Icon;
                  return (
                    <div key={s.id} className="ucg-card">
                      {/* Thumb */}
                      <div className="ucg-thumb" style={{ background: cfg.gradient }}>
                        <div className="ucg-thumb-icon"><Icon /></div>
                      </div>
                      {/* Body */}
                      <div className="ucg-body">
                        ...
                        <div className="ucg-meta">
                          ...
                          <span>{s.isAnonymous ? "anônima" : "identificada"}</span>
                        </div>
                        {/* Footer */}
              ...
              visibleModules.length === 0
                ...
                : visibleModules.map((m: any) => (
                  <div key={m.id} className="ucg-card">
                    <div className="ucg-thumb" style={{ background: MODULE_GRADIENT }}>
                      <div className="ucg-thumb-icon"><BookOpen /></div>
                    </div>
                    <div className="ucg-body">
                      <span className="ucg-cat" style={{ background: "rgba(245,158,11,.12)", color: "#D97706" }}>Programa Preventivo</span>
            ...
            {!listQ.isLoading && tab === "decompression" && (
              visibleDecomp.length === 0
                ...
                : visibleDecomp.map((a: any) => {
                  const cfg = DECOMP_CFG[a.type] ?? DECOMP_CFG[a.category] ?? { label: a.type ?? "Outro", gradient: "linear-gradient(135deg,#10B981,#059669)", catBg: "rgba(16,185,129,.12)", catColor: "#059669", Icon: Leaf };
                  const Icon = cfg.Icon;
                  return (
                    <div key={a.id} className="ucg-card">
                      ...
                      <div className="ucg-body">
                        ...
                        <div className="ucg-meta">
                          <span><Clock />{Number(a.durationMinutes ?? 0)} min</span>
                        </div>
                        <div className="ucg-footer">
                          <button
                            className="ucg-footer-btn ucg-footer-primary"
