...
import { FileText, Save, Loader2, Building2, Sparkles, Globe2 } from "lucide-react";
...
export default function AdminSesmtDefaults() {
  ...
  const enabled = scope === "global" ? isGlobal : (!isGlobal || !!pickedCompanyId);
  const queryInput = { docType, scope, ...(scope === "company" && isGlobal && pickedCompanyId ? { companyId: pickedCompanyId } : {}) };
  ...
  const saveMut = trpc.sesmt.saveDefaultTextsV2.useMutation({
    onSuccess: (r: any) => {
      if (r?.appliedToCompanies) toast.success(`Texto global salvo e aplicado a ${r.appliedToCompanies} empresa(s).`);
  ...
  function save() {
    if (scope === "company" && isGlobal && !pickedCompanyId) { toast.error("Selecione a empresa."); return; }
    saveMut.mutate({
      docType, scope,
      ...(scope === "company" && isGlobal && pickedCompanyId ? { companyId: pickedCompanyId } : {}),
      ...
      <div className="max-w-5xl mx-auto px-6 py-6 space-y-5">
        <header>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Cadastre Introdução e Conclusão padrão para PGR, Laudo Psicossocial, AEP e LTCAT.
            Cada documento gerado herda essa cópia, ficando totalmente editável.
          </p>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div>
            ...
            <select value={docType} onChange={e => setDocType(e.target.value as DocKey)} className="w-full border rounded px-3 py-2 bg-white">
          ...
          {isGlobal && (
            <div>
              ...
              <select value={scope} onChange={e => setScope(e.target.value as any)} className="w-full border rounded px-3 py-2 bg-white">
          ...
          {isGlobal && scope === "company" && (
            <div>
              <label className="block text-xs font-semibold mb-1 flex items-center gap-1"><Building2 size={12} /> Empresa</label>
              <select value={pickedCompanyId ?? ""} onChange={e => setPickedCompanyId(e.target.value ? Number(e.target.value) : null)} className="w-full border rounded px-3 py-2 bg-white">
        ...
        {!enabled && (
          <div className="text-sm text-muted-foreground p-8 text-center border border-dashed rounded-lg">
            {scope === "company" ? "Selecione a empresa para carregar e editar." : "Carregando..."}
          </div>
        )}
          ...
          <>
            {scope === "global" && (
              <div className="bg-indigo-50 border border-indigo-300 rounded-lg p-3 text-xs flex items-start gap-2">
                ...
                <div className="text-indigo-900">
                  <b>Você está editando o texto GLOBAL para "{docLabel}".</b> Usado como fallback para empresas que não tenham texto próprio cadastrado.
                </div>
              </div>
            )}
            {isSeed && (
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-xs flex items-start gap-2">
                ...
                <div className="text-amber-900">
                  <b>Texto sugestivo pré-preenchido.</b> Edite e clique em "Salvar" para fixar como padrão.
                </div>
              </div>
            )}
            {isGlobalFallback && (
              <div className="bg-sky-50 border border-sky-200 rounded-lg p-3 text-xs flex items-start gap-2">
                ...
                <div className="text-sky-900">
                  Esta empresa ainda usa o texto <b>GLOBAL</b>. Salvar agora cria uma versão própria para a empresa, sem afetar o global.
                </div>
              </div>
            )}
            ...
            <section className="bg-white border border-border rounded-xl p-4 space-y-2">
              <div className="flex items-center justify-between">
                <h2 className="font-semibold text-foreground">1. Introdução — {docLabel}</h2>
            ...
            <section className="bg-white border border-border rounded-xl p-4 space-y-2">
              <div className="flex items-center justify-between">
                <h2 className="font-semibold text-foreground">2. Conclusão — {docLabel}</h2>
            ...
            {scope === "global" && (
              <div className="bg-white border-2 border-indigo-200 rounded-xl p-4 space-y-2">
                <div className="text-sm font-semibold text-indigo-900">Aplicação do texto global</div>
                <label className="flex items-start gap-2 text-sm cursor-pointer">
                  <input type="checkbox" checked={applyToCurrent} onChange={e => setApplyToCurrent(e.target.checked)} className="mt-0.5" />
                  <span><b>Aplicar aos clientes ATUAIS</b> — sobrescreve o texto próprio de todas as empresas ativas com esta versão global.</span>
                </label>
                <label className="flex items-start gap-2 text-sm cursor-pointer">
                  <input type="checkbox" checked={applyToFuture} onChange={e => setApplyToFuture(e.target.checked)} className="mt-0.5" />
                  <span><b>Aplicar aos clientes FUTUROS</b> — empresas novas ou que não tenham texto próprio usam este global automaticamente.</span>
                </label>
              </div>
            )}
