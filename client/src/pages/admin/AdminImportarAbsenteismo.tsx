...
export default function AdminImportarAbsenteismo() {
  const [, navigate] = useLocation();
  const [rows, setRows] = useState<Row[]>([]);
  const [fileName, setFileName] = useState<string>("");
  const mut = trpc.ehs.importAbsences.useMutation();
  ...
  return (
    <AppLayout>
      <div className="max-w-5xl mx-auto p-6 space-y-6">
        ...
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2"><FileText size={22}/> Importar Absenteísmo</h1>
          <p className="text-sm text-muted-foreground mt-1">CSV com colunas: matricula, data_inicio, data_fim, motivo, cid, atestado_url</p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-border p-6 space-y-4">
          ...
          <div className="flex gap-2">
            <Button onClick={doImport} disabled={rows.length === 0 || mut.isPending}>
              <Upload size={14} className="mr-1"/> Importar {rows.length > 0 ? `${rows.length} registro(s)` : ""}
            </Button>
          </div>
        </div>
      </div>
